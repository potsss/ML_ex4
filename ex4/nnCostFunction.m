...
Ylable = zeros(m, num_labels);	%5000*10
for i = 1:m
	Ylable(i, y(i)) = 1;
end
...
J = sum(sum((-Ylable.*log(a3))- (1 - Ylable).*log(1 - a3)))/m;

J = J + ...
	(lambda * (sum(sum(Theta1(:, 2:end).^2)) + sum(sum(Theta2(:, 2:end).^2))))...
	/ (2*m);

% Backpropogation
delta3 = a3 - Ylable; %5000*10
